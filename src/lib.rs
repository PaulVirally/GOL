...
static VERT_SHADER_SRC: &str = r#"#version 300 es
    ...
    layout(location=1) in float attr_color;
    out float out_color;
    void main() {
...
static FRAG_SHADER_SRC: &str = r#"#version 300 es
    ...
    out vec4 frag_color;
    void main() {
        frag_color = vec4(out_color, out_color, out_color, 1.f);
    }
"#;
...
pub fn main() -> Result<(), JsValue> {
    ...
    let vert_shader = compile_shader(&context, WebGl2RenderingContext::VERTEX_SHADER, VERT_SHADER_SRC)?;
    let frag_shader = compile_shader(&context, WebGl2RenderingContext::FRAGMENT_SHADER, FRAG_SHADER_SRC)?;
    ...
    grid.init_gl(&context, win_size.0 as f32, win_size.1 as f32);
